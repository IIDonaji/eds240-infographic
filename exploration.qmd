...
```{r}
# Citizen science stats
length(unique(bats_clean$user_login))  # Total observers
nrow(bats_clean)  # Total observations
range(bats_clean$year)  # Year range
```

# V. Answer questions

notes: 
Standarise the observations counts instead of raw counts observation per by survey efforts by observations given / count maybe %
...
Plot 1: The month observation plot shows a very high peak in April and then peaks again in August. Something about early spring and late summer influences people to encounter/ observe more bats. Plot 2: Observation peaked in 2017-2019, with sparse data before 2014 and a drop after 2020. I might decide to focus on observations from 2015 to 2025 since it has clearer distributions.
